...
dbutils.widgets.text(
    "experiment_name",
    f"/dev-mlops_stacks_demo-experiment",
...
dbutils.widgets.text(
    ...
    "dev.cicd_proj.trip_pickup_features",
...
dbutils.widgets.text(
    ...
    "dev.cicd_proj.trip_dropoff_features",
...
# input_table_path, experiment_name, model_name
...
# Train a lightGBM model with evaluation set
model = lgb.train(
    param, 
    train_lgb_dataset, 
    num_rounds, 
    valid_sets=[test_lgb_dataset],  # Include the test dataset as an evaluation set
    valid_names=['eval']  # Naming the evaluation set
)

# COMMAND ----------

# DBTITLE 1,extract current run model_uri
# Get the current run's artifact URI
artifact_uri = mlflow.get_artifact_uri()

# Assuming a model is logged as 'model' within the run
model_uri = artifact_uri + "/model"
print("Model URI:", model_uri)

# COMMAND ----------

# DBTITLE 1,Include evaluation metrics for monitoring
test_data_df = test.copy()
test_data_df['true_labels'] = test['fare_amount']

results = mlflow.evaluate(
        model=model_uri,  # This should be the model object or the URI to the logged model
        # data=test_lgb_dataset, #eval_data,
        data=test_data_df,  # Now passing a Pandas DataFrame
        targets='true_labels',
        model_type='regressor',  # Specify the model type, e.g., 'regressor' for regression models
        # Add any additional configuration or metrics specific to your model or domain
        # extra_metrics=[mymetric],  # Add the custom metric
    )
